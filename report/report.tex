...
\begin{equation*}
    \dot{\Phi}(X(t)) = \frac{d}{dt} \Phi\left(X(t)\right) = \left(\nabla\Phi(X)\right)^T \frac{dX}{dt} = -\Vert\nabla\Phi(X)\Vert_F^2 \le 0.
\end{equation*}
Observe $\dot{\Phi} \le 0$ is negative semi-definite, so therefore $\Phi$ is a Lyapunov function for this dynamical system, and all zero solutions of the matrix ODE are stable. Moreover, we have that $\dot{\Phi}(X) = 0$ exactly when the ODE RHS $-\nabla\Phi(X) = 0.$
